...
async def download_file(client: TelegramClient, message: Message, output_file: str, dest_folder, progress_callback: Callable[[int, int], Any] | None=None, completed_callback: Callable[[], Any] | None=None):

    # Folder handling
    dest = dest_folder
    output_file_stem = output_file.split(".")[0]
    makedirs(f'{dest}/{output_file_stem}', exist_ok=True)
    ...
    # Temporary file
    temp_file = f"{dest}/{output_file_stem}/{output_file}_{crypt(15)}.bin"
